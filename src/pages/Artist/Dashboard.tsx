...
const Dashboard: React.FC = () => {
  ...
  const [selectedArtworkForEdit, setSelectedArtworkForEdit] = useState<Artwork | null>(null);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedArtworkForEdit || !currentUser) return;
    ...
    try {
      ...
      if (editFormData.file) {
        ...
        const compressedFile = await compressImage(editFormData.file);
        ...
        updateData.imageUrl = mainResult.secure_url;
        updateData.thumbnailUrl = thumbResult.secure_url;
        updateData.publicId = mainResult.public_id;
      }

      // Update in Firestore
      await updateDoc(doc(db, 'artworks', selectedArtworkForEdit.id), updateData);
      
      // Update local state
      setArtworks(artworks.map(artwork => 
        artwork.id === selectedArtworkForEdit.id 
          ? { ...artwork, ...updateData }
          : artwork
      ));
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        {/* Edit Modal */}
        {showEditModal && selectedArtworkForEdit && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-2xl w-full p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4 sm:mb-6">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Artwork</h2>
                <button
                  onClick={() => {
                    setShowEditModal(false);
                    setSelectedArtworkForEdit(null);
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4 sm:space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <img
                      src={selectedArtworkForEdit.thumbnailUrl || selectedArtworkForEdit.imageUrl}
                      alt={selectedArtworkForEdit.title}
                ...
                <FileUpload
                  onFileSelect={(file) => setEditFormData({...editFormData, file})}
                  selectedFile={editFormData.file}
                  accept="image/*"
                  label="Replace Image (Optional)"
                  description="Upload a new image to replace the current one (JPEG, PNG, WebP)"
                  disabled={updating}
                />
                ...
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    id="editCustomizable"
                    checked={editFormData.isCustomizable}
                    onChange={(e) => setEditFormData({...editFormData, isCustomizable: e.target.checked})}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <label htmlFor="editCustomizable" className="ml-2 text-sm text-gray-700">
                    This artwork can be customized
                  </label>
                </div>
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                  ...
                  >
                    ...
                    <span>{updating ? 'Updating...' : 'Update Artwork'}</span>
                  </button>
                </div>
              </form>
            </div>
