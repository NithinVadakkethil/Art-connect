...
const Dashboard: React.FC = () => {
  ...
  const [selectedRequirement, setSelectedRequirement] =
    useState<SharedRequirement | null>(null);
  const [selectedOrder, setSelectedOrder] = useState<SharedOrder | null>(null);
  const [uploading, setUploading] = useState(false);
  const [declineReason, setDeclineReason] = useState("");
  ...
  const filterArtworks = () => {
    ...
    if (artworkSearchTerm) {
      filtered = filtered.filter(
        (artwork) =>
          artwork.title
            .toLowerCase()
            .includes(artworkSearchTerm.toLowerCase()) ||
          artwork.description
            .toLowerCase()
            .includes(artworkSearchTerm.toLowerCase()) ||
          artwork.tags.some((tag) =>
            tag.toLowerCase().includes(artworkSearchTerm.toLowerCase())
          )
      );
    }
    ...
    if (artworkCategoryFilter !== "all") {
      filtered = filtered.filter(
        (artwork) => artwork.category === artworkCategoryFilter
      );
    }
  ...
  const filterRequirements = () => {
    ...
    if (requirementSearchTerm) {
      filtered = filtered.filter(
        (requirement) =>
          requirement.requirement.clientName
            .toLowerCase()
  ...
  const filterOrders = () => {
    ...
    if (orderSearchTerm) {
      filtered = filtered.filter(
        (order) =>
          order.order.artwork.title
            .toLowerCase()
            .includes(orderSearchTerm.toLowerCase()) ||
          order.order.artwork.category
            .toLowerCase()
            .includes(orderSearchTerm.toLowerCase())
      );
    }
  ...
  const handleRequirementAction = async (
    ...
    if (
      selectedRequirement.requirement.acceptedBy &&
      selectedRequirement.requirement.acceptedBy.artistId !== currentUser.uid
    ) {
      toast.error(
        "This requirement has already been accepted by another artist"
      );
      return;
    }
    ...
    try {
      ...
      if (action === "accept") {
        ...
        await updateDoc(
          doc(db, "requirements", selectedRequirement.requirementId),
          {
            status: "assigned",
            acceptedBy: {
              artistId: currentUser.uid,
        ...
        const allSharedRequirementsSnapshot = await getDocs(
          allSharedRequirementsQuery
        );

        const updatePromises = allSharedRequirementsSnapshot.docs.map(
          async (sharedDoc) => {
  ...
  const deleteRequirement = async (requirementId: string) => {
    ...
    try {
      await deleteDoc(doc(db, "sharedRequirements", requirementId));
      setSharedRequirements(
        sharedRequirements.filter((req) => req.id !== requirementId)
      );
      toast.success("Requirement deleted successfully");
      setShowRequirementModal(false);
    } catch (error) {
  ...
  const isRequirementAcceptedByOther = (requirement: SharedRequirement) => {
    return (
      requirement.requirement.acceptedBy &&
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Manage your artworks, view client requirements, and handle orders in your artist dashboard."
        />
      </Helmet>
            ...
            <div className="p-4 sm:p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div
                    className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 ${
                      showArtworkFilters ? "block" : "hidden sm:grid"
                    }`}
                  >
                    <div className="relative">
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        value={artworkCategoryFilter}
                        onChange={(e) =>
                          setArtworkCategoryFilter(e.target.value)
                        }
                      >
                        <option value="all">All Categories</option>
                        {categories.map((category) => (
              ...
              {activeTab === "requirements" && (
                <div className="space-y-4">
                  ...
                  <div className="sm:hidden">
                    <button
                      onClick={() =>
                        setShowRequirementFilters(!showRequirementFilters)
                      }
                      className="w-full flex items-center justify-between p-3 bg-gray-50 rounded-lg text-sm font-medium text-gray-700"
                    >
                      <span>Filters</span>
                  ...
                  <div
                    className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 ${
                      showRequirementFilters ? "block" : "hidden sm:grid"
                    }`}
                  >
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4 sm:h-5 sm:w-5" />
                  ...
                  {filteredRequirements.length === 0 ? (
                    ...
                    filteredRequirements.map((requirement) => {
                      const isAcceptedByOther =
                        isRequirementAcceptedByOther(requirement);
                      const isAcceptedByMe =
                        isRequirementAcceptedByMe(requirement);
              ...
              {activeTab === "orders" && (
                <div className="space-y-4">
                  ...
                  <div
                    className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 ${
                      showOrderFilters ? "block" : "hidden sm:grid"
                    }`}
                  >
                    <div className="relative">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4 sm:h-5 sm:w-5" />
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 space-y-2 sm:space-y-0">
                          <div className="flex items-center space-x-3 sm:space-x-4">
                            <div
                              ...
                              onClick={() =>
                                openImageModal(order.order.artwork.imageUrl)
                              }
                            >
                              <img
                                src={order.order.artwork.imageUrl}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xl sm:text-2xl font-bold text-green-600">
                        ₹{selectedRequirement.proposedPrice}
                      </p>
                    </div>
                    ...
                    {selectedRequirement.requirement.deadline && (
                      <div>
                        ...
                        <p className="text-gray-700 text-sm sm:text-base">
                          {new Date(
                            selectedRequirement.requirement.deadline.seconds *
                              1000
                          ).toLocaleDateString()}
                        </p>
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        {selectedRequirement.status === "declined" &&
                        selectedRequirement.declineReason
                          ? "Declined by you"
                    ...
                    {isRequirementAcceptedByMe(selectedRequirement) && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
                        ...
                        <p className="text-green-700 text-sm">
                          You accepted this requirement on{" "}
                          {formatTimestamp(
                            selectedRequirement.requirement.acceptedBy
                              ?.acceptedAt
                          )}
                    ...
                    {isRequirementAcceptedByOther(selectedRequirement) && (
                      <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 sm:p-4">
                        ...
                        <p className="text-orange-700 text-sm">
                          This requirement has been accepted by Others
                          {/* <strong>
                            {
                              selectedRequirement.requirement.acceptedBy
                                ?.artistName
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  <div>
                    <div className="relative">
                      <img
                        ...
                        className="w-full h-auto object-contain max-h-96 rounded-lg cursor-pointer"
                        onClick={() =>
                          openImageModal(selectedOrder.order.artwork.imageUrl)
                        }
                      />
                      <button
                        onClick={() =>
                  ...
                  <div className="space-y-4">
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">
                        {selectedOrder.order.artwork.title}
                      </h3>
                      <p className="text-gray-600 text-sm sm:text-base">
                        {selectedOrder.order.artwork.category}
                      </p>
                    </div>
